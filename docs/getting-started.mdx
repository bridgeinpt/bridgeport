...
![The BRIDGEPORT dashboard — your servers, services, and their connections at a glance](images/dashboard.png)
...
```bash title="Start BRIDGEPORT (quick try-out)"
docker run -d \
  --name bridgeport \
  -p 3000:3000 \
...
<Tabs>
<TabItem label="Same machine (Docker socket)">
...
```bash title="Re-run with the Docker socket mounted"
docker run -d \
  --name bridgeport \
  -p 3000:3000 \
  -v bridgeport-data:/data \
...
<TabItem label="Remote server (SSH)">
