...
// Short-TTL, single-flight caches for the full-window (non-delta) metrics
// history reads. Concurrent identical requests collapse onto one compute, and
// the result is reused for a few seconds — see lib/response-cache.ts. Delta
// requests (per-client `since`) are never cached. A new metrics sample only
// lands every few minutes, so a 5s window is well within chart tolerance.
const serverHistoryCache = createResponseCache<unknown>();
const serviceHistoryCache = createResponseCache<unknown>();
...
export async function monitoringRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      // The whole DB read + columnar transform, returning the response payload
      // or null when the env is missing. Wrapped so the full-window path can be
      // served through the single-flight cache while the delta path runs it
      // directly (per-client `since`, not cacheable).
      const compute = async () => {
      ...
      // A cached full-window payload carries the `until` from when it was
      // computed (up to the TTL old). That's safe: `until` is captured before
      // the read precisely so the next delta "can never skip rows" — serving an
      // older `until` only makes that delta re-fetch a few extra seconds (strict
      // `>`, client aligns by timestamp), never opens a gap.
      const payload = isDelta
        ? await compute()
        : await serverHistoryCache.getOrCompute(
            `${envId}:${metric ?? 'all'}:${hours}:${maxPoints}`,
            compute
          );
      if (!payload) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      // Full DB read + columnar transform, returning the payload or null when
      // the env is missing. Full-window requests are served through the
      // single-flight cache; delta requests run it directly (uncacheable).
      const compute = async () => {
      ...
      // Cached payloads carry a slightly-old `until` — safe for the same reason
      // as the server-history route above (older `until` → next delta re-fetches
      // a few seconds, never skips).
      const payload = isDelta
        ? await compute()
        : await serviceHistoryCache.getOrCompute(
            `${envId}:${hours}:${maxPoints}`,
            compute
          );
      if (!payload) {
        return reply.code(404).send({ error: 'Environment not found' });
